...
@AndroidEntryPoint
class HomeScreen : Fragment(), ItemClicked, OnNavigationItemSelectedListener {
    ...
    private fun setUpCategory() {
        ...
        adapter.currentIndex().observe(viewLifecycleOwner) { value: Int ->
            binding.categoryListRecycler.adapter = when (value) {
                0 -> {
                    if (nowPlayingAdapter.pageCurrent() == 1)
                        loadData {
                ...
                1 -> {
                    if (popularAdapter.pageCurrent() == 1)
                        loadData {
                ...
                2 -> {
                    if (upComingAdapter.pageCurrent() == 1)
                        loadData {
                    ...
                    when (adapter.currentIndex().value) {
                        0 -> {
                            ...
                            if (page != -1 && page != 1)
                                ...
                                if (page == -1)
                                    binding.categoryListRecycler.scrollToPosition(0)
                        }
